...
#            pdb.set_trace()
            ...
            if images[i].dtype == np.uint8:
                ...
                datum.datum.data = images[i].transpose(2, 0, 1).tostring()
            else:
...
for i in range(6000):
    ...
    new_img = np.zeros((rgb_img.shape[0],rgb_img.shape[1],6),dtype=np.uint8)
    new_img[:,:,0:3] = rgb_img
    new_img[:,:,3:6] = dhi_img
