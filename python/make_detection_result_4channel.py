...
def load_labels():
  ...
  for data_idx in range(s_idx,f_idx,1):
    gt_label = []
    label_path = (base_dir + 'SSD/caffe/data/KITTI/label_2/' + '%06d' % (data_idx) + '.txt')
...
def load_predictions_3channel():
  ...
  for data_idx in range(s_idx,f_idx,1):
    ...
    transformer.set_mean('data', np.array([123,117,104,122]))
    transformer.set_channel_swap('data', (2,1,0,3))
#    transformer.set_raw_scale('data', 255.0)
    net.blobs['data'].reshape(1,4,test_input_ysize,test_input_xsize)
...
#    dhi_img = caffe.io.load_image('/media/user/4b3dfae8-c6b6-4a03-936d-d8fee7ff0b89/SSD/caffe/data/KITTI/depth_image/python_sparse/sparse_depthheightintensity/'+'%06d.png'%(data_idx))
    depth_img = cv2.imread('/media/user/4b3dfae8-c6b6-4a03-936d-d8fee7ff0b89/SSD/caffe/data/KITTI/depth_gain_test/4channel_sparse_img/'+'%06d.png'%(data_idx),cv2.IMREAD_UNCHANGED)
    new_img = np.zeros((frame.shape[0], frame.shape[1], 4), dtype=np.float32)
    new_img[:,:,0] = frame[:,:,0]*255
    new_img[:,:,1] = frame[:,:,1]*255
    new_img[:,:,2] = frame[:,:,2]*255
    new_img[:,:,3] = depth_img[:,:,3]
#    new_img[:,:,4] = dhi_img[:,:,1]
#    new_img[:,:,5] = dhi_img[:,:,2]
    transformed_image = transformer.preprocess('data',new_img)
