...
void AnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  cv::Mat cv_img1;
  const string& data1 = anno_datum.datum().data();
  std::vector<char> vec_dataa(data1.c_str(), data1.c_str() + data1.size());
  cv_img1 = cv::imdecode(vec_dataa, -1);
    ...
    timer.Start();
  cv::Mat cv_img;
  const string& data = sampled_datum->datum().data();
